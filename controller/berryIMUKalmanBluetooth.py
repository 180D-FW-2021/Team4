...
while True:
    ...
    expo = 1.1
    #expo < 1 makes more initially sensitive, expo > 1 makes less initially sensitive
    
    minout = 3
    #minimum steering output
    
    if (-maxang < kalmanX and kalmanX < maxang):
        if (kalmanX < 0):
            output = -int(((((-kalmanX)//res)/steps) ** expo) * steps)
        else:
            output = int((((kalmanX//res)/steps) ** expo) * steps)
